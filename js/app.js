...
const languagesItems = languagesMenu.querySelectorAll('span');
const languagesItem1 = languagesItems[0];
const languagesItem2 = languagesItems[1];
const lanlanguagesList = languagesMenu.querySelectorAll('li');


if(document.getElementById('languagesMenu')){
	if(!languagesItem1.matches('._active')){
		languagesMenu.addEventListener('click', changeLanguages)
	}
	
}

function changeLanguages(){
	languagesMenu.removeEventListener('click', changeLanguages)
	languagesItem1.classList.add('_active');
	languagesItem2.classList.add('_active');
	for (language of lanlanguagesList){
		if(language.matches('._active')){
			language.style.cursor = 'auto';
			language.style.color = 'red';
		}else{
			language.style.fontSize = 'inherit';
		}
	}
	document.addEventListener('click', closeChangeLanguage)
}

function closeChangeLanguage(e){
	for (language of lanlanguagesList){
		if(e.target === language){

		} else {
			languagesItem1.classList.remove('_active');
			languagesItem2.classList.remove('_active');
		}
	}
}
